...
{
	if (empty($GLOBALS['current_db_transaction']))
	{
		execute("START TRANSACTION");

		$key = microtime();

		$GLOBALS['current_db_transaction'] = $key;

		return $key;
	}
}

function endTransaction($key)
{
	if (empty($GLOBALS['current_db_transaction']) || $key === $GLOBALS['current_db_transaction'])
	{
		execute("COMMIT");
	}
}
